...
| | SELECT * FROM people WHERE user_id like "%bc%" | db.people.find( { user_id: /bc/ } )  hoặc db.people.find( { user_id: { $regex: /bc/ } } )|
| | SELECT COUNT(user_id) FROM people | db.people.count( { user_id: { $exists: true } } ) hoặc db.people.find( { user_id: { $exists: true } } ).count() |
